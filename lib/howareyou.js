(function ($) {
  ...
  $(function () {
    ...
    if (getQueryParameter('authorized')) {
      user = JSON.parse(getQueryParameter('data'));
    }
    ...
    //   authenticate(data, true, function (data) {
    //     form.reset();
    //     login_status.text('');
    //     hide_current();
    //   }, function (error) {
    ...
    //     hide($('#howareyou_register_modal'));

    //     var auth_data = data.filter(function (param) {
    //       return param.name === 'email' || param.name === 'password';
    //     }).concat([{ name: 'app_id', 'value': APP_KEYS.id }]);

    //     authenticate(auth_data, true);
    //   }, function (error) {
    //     register_status.text(
  ...
  function authenticate (data, real, success, error) {
    post(AUTHENTICATE_ENDPOINT, data, function (data) {
      var new_user = get_user(data);
      if (real) {
        user = new_user;
      } else {
        demo_user = new_user;
